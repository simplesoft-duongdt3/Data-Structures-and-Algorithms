...
class SinglyLinkedList<T> {
  ...
  void forEach(void Function(SinglyNode<T>) action) {
...
int calRecursive(SinglyNode<int>? node) {
  if (node == null) {
    return 1;
  }

  var sum = calRecursive(node.next);
  var newData = node.data + sum;
...
void main() {
  ...
  var remaining = calRecursive(list.head);
  if (remaining > 0) {
    //addHead
  }

  Stack<SinglyNode<int>> stack = Stack();
  list.forEach((node) {
    stack.push(node);
  });

  int addValue = 1;
  int remain = 0 + addValue;
  while (stack.isNotEmpty()) {
    var node = stack.pop();

    var sum = node.data + remain;
